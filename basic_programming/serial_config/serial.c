 ...
 */
int send_data(int fd, char *buffer, int size, int speed) {
...
int main(int argc, char *argv[]){
	...
	if(m5_uart_Config(fd, speed, "n81", 232) == -1)
		return -1;
	
	send_data(fd, "Hello World!!", 13, speed);

	bzero(buf, 256);
	recv = recv_data(fd, buf, 256, 1000000); // wait 1 second
